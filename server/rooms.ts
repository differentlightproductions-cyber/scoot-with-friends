...
type Room={map?:string;mapRevision:string;code:string;owner:string;locked:boolean;generation:string;players:Map<string,Player>};
export function createRooms({port=8787,host='127.0.0.1',capacity=2,grace=30000,origins=['http://127.0.0.1:5182','http://localhost:5182']}={}){
 ...
 const remove=(r:Room,p:Player)=>{r.players.delete(p.id);if(r.owner===p.id)r.owner=[...r.players.values()].find(x=>x.ws)?.id??[...r.players.keys()][0]??'';if(!r.players.size)rooms.delete(r.code);else broadcast(r,roster(r));};
  ...
  ws.on('message',raw=>{try{
   ...
   if(!player){
    ...
    if(m.type==='resume'){
     ...
     if(m.type==='create'){if(rooms.size>=32){error('Server full');return;}room={map:'outdoor',mapRevision:m.mapRevision,code:token(),owner:'',locked:false,generation:randomUUID(),players:new Map()};rooms.set(room.code,room);}else if(m.type==='join')room=rooms.get(m.code)??null;else return;
     if(!room||(room.map==='outdoor'&&room.mapRevision!==m.mapRevision)||room.locked||room.players.size>=capacity){error('Room unavailable, locked or full.');return;}
   ...
   if(m.type==='map'){
    ...
    if(!['outdoor','techno_gravity','b_hill','church'].includes(m.map)||!/^([a-f0-9]{64})$/.test(m.mapRevision??'')){error('Unsupported room destination');return;}
   ...
   if(m.type==='ready'){
    ...
    player.ready=true;send(ws,{type:'ready',generation:room.generation});broadcast(room,roster(room));return;
   }
