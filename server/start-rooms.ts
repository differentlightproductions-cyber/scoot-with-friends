...
const service=createRooms({port,host:process.env.ROOM_HOST??'127.0.0.1',capacity:Number(process.env.ROOM_CAPACITY??8),origins:(process.env.ROOM_ORIGINS??'http://127.0.0.1:5182,http://localhost:5182').split(','),socialFile:process.env.ROOM_SOCIAL_FILE??''});
