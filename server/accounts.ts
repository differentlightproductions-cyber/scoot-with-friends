...
export class Accounts {
 ...
 equip(id:string,build:ScooterLoadout,revision:number){return this.atomic(()=>{const r=this.row(id);if(r.stage!=='ready_to_ride'||r.revision!==revision)throw Error('Setup changed. Reload and retry');const scooter=this.validBuild(build);for(const item of Object.values(scooter)){const part=PARTS.find(p=>p.id===item.partId)!;if((part.unlockType!=='free'||part.variants.find(v=>v.id===item.variantId)?.exclusive)&&!this.db.prepare('SELECT 1 FROM ownership WHERE account_id=? AND part_id=? AND variant_id=?').get(id,item.partId,item.variantId))throw Error('Part not owned');}this.db.prepare('UPDATE accounts SET loadout=?,revision=revision+1 WHERE id=?').run(JSON.stringify(scooter),id);return this.get(id);});}
 close(){this.db.close();}
}
