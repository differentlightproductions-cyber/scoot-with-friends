...
export class WarehouseBuilder {
 ...
 begin(o:ParkObject){this.placement=structuredClone(o);this.cursor.set(o.x,o.y,o.z);this.angle=o.rotation;
  ...
  this.ghost=buildObject(temp,o);temp.world.free();
  this.ghost.traverse(p=>{if(p instanceof THREE.Mesh){for(const m of Array.isArray(p.material)?p.material:[p.material])m.dispose();p.material=new THREE.MeshBasicMaterial({color:0x96d7bd,transparent:true,opacity:.4,wireframe:false,depthWrite:false});}});this.park.scene.add(this.ghost);}
 cancel(){if(this.ghost){this.ghost.removeFromParent();this.ghost.traverse(o=>{if(o instanceof THREE.Mesh){o.geometry.dispose();(o.material as THREE.Material).dispose();}});}this.ghost=null;this.placement=null;this.prompt.hidden=true;}
