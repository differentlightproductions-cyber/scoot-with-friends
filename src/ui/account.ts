...
export class AccountPanel {
  ...
  private renderChoice() {
    ...
    const card = (title: string, s: { level: number; credit: number; parts: number }, note: string) => `<div class="account-save"><strong>${title}</strong><span>Level ${s.level} · ${s.credit.toLocaleString('en-US')} Coins · ${s.parts} parts</span><small>${note}</small></div>`;
