...
export class FreeRide {
 ...
 async connect(mode:'create'|'join'|'resume',name='Rider',code=''){
  if(mode!=='resume'){if(['Connecting','Loading','Reconnecting'].includes(this.status))return;this.leave();this.status='Connecting';this.lastError='';this.onChange();await this.prepare();this.map='outdoor';}
  ...
  ws.onopen=()=>this.send({type:mode,mapRevision,protocol:PROTOCOL,content:CONTENT,name,code:mode==='resume'?this.code:code,secret:mode==='resume'?this.secret:undefined,appearance:this.profile,socialCredential:this.socialCredential,playfulContact:this.profile.settings.playfulContact});
  ws.onmessage=e=>{if(this.ws!==ws)return;let m;try{m=JSON.parse(e.data);}catch{return;}
   ...
   if(m.type==='social-error'||m.type==='error'){this.lastError=m.message;if(!this.id)this.status=m.message;this.onChange();return;}
