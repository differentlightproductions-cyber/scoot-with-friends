...
The local browser pass attempted all 79 discovered `tests/*.browser.mjs` suites: **54 pass, 22 fail, 2 timeout, 1 unverified** after targeted reruns. Every sweep process had a 65-second cap. Detailed output is in `artifacts/regression/*.browser.mjs.log`; `summary.json` records the initial sweep before the noted reruns.
...
- `weather.browser.mjs` checks stored weather/time values and the visible selected values in the menu rows. It passed 18/18 with a clean process exit after the locale edits.
...
| weather.browser.mjs | PASS | 18/18; settings assert stored values; direct rerun. |
