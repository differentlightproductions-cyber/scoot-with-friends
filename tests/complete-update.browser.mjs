...
try{
 ...
 const result=await page.evaluate(async()=>{
  ...
  g.startSession('outdoor',true);s=(()=>{const r=g.interactions.items.find(i=>i.id==='rack-0').position;return place(r.x,r.z-1);})();s.walking=true;f=emptyInput();f.pressed.brakeBars=true;g.interactions.update(s,f,.02);check('Rack starts storing real scooter',!!g.interactions.stored&&!s.hasScooter);g.interactions.update(s,emptyInput(),.8);g.interactions.update(s,f,.02);g.interactions.update(s,emptyInput(),.8);check('Rack retrieves scooter',!g.interactions.stored&&s.hasScooter);
