...
try{
 ...
 await page.waitForFunction(()=>window.__LAZER?.rider.avatar,null,{timeout:90000});
 ...
 const statesResult=await page.evaluate(async()=>{
  ...
  const pose=(name)=>{
   ...
   const ghosts=()=>{let n=0;r.avatar.group.traverse(o=>{if(o.isMesh&&!o.material.colorWrite)n++;});return n;};let masked=null;for(let i=0;i<40;i++){r.update(s,1/60,1);r.avatar.setFirstPerson(true);masked=ghosts();r.root.updateMatrixWorld(true);g.camera.update(s,{rx:0,ry:0,held:{},pressed:{}},1/60,1);}g.renderer.render(g.park.scene,g.camera.camera);
  ...
  const beforeBones=[r.avatar.head,r.avatar.chest];r.root.updateMatrixWorld(true);const before=beforeBones.map(b=>b.getWorldPosition(new T.Vector3()));r.hideHead=false;g.camera.view='third';r.update(s,1/60,1);r.avatar.setFirstPerson(false);r.root.updateMatrixWorld(true);const after=beforeBones.map(b=>b.getWorldPosition(new T.Vector3()));let ghostsLeft=0;r.avatar.group.traverse(o=>{if(o.isMesh&&!o.material.colorWrite)ghostsLeft++;});const restored={coreMask:ghostsLeft,boneMovement:Math.max(0,...before.map((p,i)=>p.distanceTo(after[i])))};return {states:out,restored};
 });
