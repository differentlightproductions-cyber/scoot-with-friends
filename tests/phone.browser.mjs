...
try {
  ...
  const ring = await run(() => { const g = window.__LAZER, el = document.querySelector("#phone-hold"), seen = () => !!el && !el.hidden; const out = {}; g.phoneStep({ held: { menuDown: 1 }, pressed: { menuDown: true } }); for (let i = 0; i < 7; i++) g.phoneStep({ held: { menuDown: 1 } }); out.tap = seen(); for (let i = 0; i < 12; i++) g.phoneStep({ held: { menuDown: 1 } }); out.mid = seen(); out.fill = +(el?.style.getPropertyValue("--p") || 0); g.phoneStep({ released: { menuDown: true } }); out.released = seen(); return out; });
  check("14 A ring fills while D-pad Down is held, not for a tap, gone on release", !ring.tap && ring.mid && ring.fill > 0.45 && ring.fill < 0.8 && !ring.released, ring);
  await run(() => window.__LAZER.phone.stow());
