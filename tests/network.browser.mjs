...
try{const pages=[];for(let i=0;i<3;i++){const c=await b.newContext(),p=await c.newPage();p.on('pageerror',e=>errors.push(e.message));await p.goto('http://127.0.0.1:5182');await p.waitForFunction(()=>window.__LAZER?.rider.avatar);pages.push(p);}
