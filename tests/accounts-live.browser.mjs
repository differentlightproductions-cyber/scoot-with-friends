...
try{
 ...
 await dialog.getByRole('button',{name:'Sync now'}).click();await dialog.locator('[role=status]').filter({hasText:'saved to your account'}).waitFor({timeout:30000});await page.reload();await page.locator('#destination-loading').waitFor({state:'hidden',timeout:60000});
 ...
 const cloud=await page.evaluate(async()=>await(await fetch('/api/account/save')).json());assert(cloud.save?.revision>=1&&cloud.save.data?.wallet,'cloud save stored');
 console.log(JSON.stringify({registrationMs,registration:true,login:true,secureCookie:true,refresh:true,reloadSession:true,cloudSave:true}));
