...
// Real hop -> A -> Decade, filmed from fixed world cameras (front, side, rear 3/4)
...
try{
 ...
 const out=await page.evaluate(async([stance,style])=>{
  ...
  const W=300,H=360,moments=[.02,.12,.22,.32,.42,.52,.62],renderer=g.renderer;
  const comp=document.createElement('canvas');comp.width=W*moments.length;comp.height=H*3;const ctx=comp.getContext('2d');
  ...
  const rows=[];let airStart=null,t=0,pressed=false,shot=0;
  ...
  for(let i=0;i<400;i++){
   ...
   let input={};
   if(airStart!==null&&!pressed&&tAir>=.02){input={pressed:{hop:true},held:{hop:1}};pressed=true;}
   a(1/120,input,shot<moments.length&&tAir>=moments[shot]-1/120);t+=1/120;
   if(airStart!==null&&shot<moments.length&&tAir>=moments[shot]){
    a(0,{},true);rows.push({tAir:+tAir.toFixed(2),...measure()});
