...
import { DISTRICTS, RIDE_SPEED, SPOTS, TRAVEL_TARGETS, districtOf, pathLength, routeBetween, toWorld } from "../src/data/world";
...
test("every fast-travel spot names a real map and spawn, and every district has a spot", () => {
  const spawns: Record<string, unknown[]> = { church: CHURCH_SPAWNS, b_hill: B_HILL_SPAWNS };
  ...
  assert.deepEqual(SPOTS.map((s) => s.label), ["VETERANS MEMORIAL PARK", "CHURCH — FRONT", "CHURCH — INTERIOR", "B HILL — SUMMIT", "B HILL — PUEBLO / BOTTOM"]);
  assert.equal(CHURCH_SPAWNS[SPOTS.find((s) => s.id === "church-interior")!.spawn].name, "SANCTUARY");
  assert.equal(B_HILL_SPAWNS[SPOTS.find((s) => s.id === "b_hill-bottom")!.spawn].name, "B HILL / RUNOUT");
});
