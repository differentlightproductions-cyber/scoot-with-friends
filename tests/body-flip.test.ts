...
test('Only a fresh manual hop or supported fastplant unlocks body rotation',()=>{
 for(const origin of ['manual_hop','natural_ramp_air','trick_initiated_pop','fastplant'] as const){
  ...
  if(flip.active){assert(flip.angle>2);const angle=flip.angle;flip.step(1/120,false,0,angle+.3);assert(flip.angle>angle);for(let i=0;i<120;i++)flip.step(1/120,true,1,flip.angle+.3);assert(flip.velocity<0);}
...
test('Flip integration remains continuous across inversions and frame rates',()=>{
 const results=[];for(const hz of [30,60,120]){const f=new BodyFlipControl();f.begin('manual_hop');for(let n=0;n<hz*2;n++)f.step(1/hz,true,-1,f.angle);results.push(f.angle);assert(f.angle>Math.PI*4);}
