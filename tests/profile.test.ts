...
test('versioned profile migrates old saves and preserves authored gear/settings',()=>{
 ...
 const p=loadProfile();assert.equal(p.version,2);assert.equal(p.riderId,'rider-02');assert.equal(p.settings.stance,'goofy');assert.equal(p.settings.sound,false);
 p.outfit.head='head-vented-forest';p.outfit.top='top-hoodie-red';p.settings.daylight='night';
