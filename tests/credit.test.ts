...
test('confirmed rewards, atomic purchases, duplicate input, storage failure and migration',async()=>{
 ...
 data.set(PROFILE_KEY,JSON.stringify({version:2,riderId:'rider-02',settings:{stance:'goofy'}}));const old=loadProfile();assert.equal(old.riderId,'rider-01');assert.equal(old.wallet.credit,0);
