...
test('confirmed rewards, atomic purchases, duplicate input, storage failure and migration',async()=>{
 ...
 const e=new CreditEconomy();let paid=0;e.onGains=g=>{paid+=g.credit;};assert.equal(await e.reward('confirmed:1',7600),'ok');await e.reward('confirmed:1',7600);assert.equal(loadProfile().wallet.credit,76);
 const part={partId:'mafioso-bars-y',variantId:'mafioso_bars_y_black_gold'};const results=await Promise.all([e.buy(part),e.buy(part)]);assert.deepEqual(results,['ok','Already owned']);assert.equal(loadProfile().wallet.credit,1+paid);assert.equal(loadProfile().progress.stats.purchases,1);assert.ok(owns(loadProfile().wallet,part));
 saveProfile(old);assert.equal(loadProfile().wallet.credit,1+paid);assert.ok(owns(loadProfile().wallet,part));
 ...
 assert.equal(await e.buy(wheel),'ok');assert.equal(loadProfile().wallet.testCredit,945);assert.equal(loadProfile().wallet.credit,1+paid);assert.ok(owns(loadProfile().wallet,wheel));
