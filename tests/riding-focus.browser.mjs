...
try{
 ...
 const result=await page.evaluate(async()=>{
  ...
  g.startSession('warehouse',true);g.sim.reset(0,true);a(.4);
  ...
  for(const ry of [.2,.5,1]){a(.12,{ry});for(let i=0;i<8;i++)g.render();crouches.push(g.rider.crouch);}
  check('Analog preload is visible at low, medium and full RS',crouches[0]>.02&&crouches[1]>crouches[0]&&crouches[2]>crouches[1],crouches);
  ...
  f.lean=-1;g.social.update(g.sim,f,.02);g.social.update(g.sim,emptyInput(),.02);check('Release selects a wave',g.sim.emote?.id==='wave');
 ...
 await page.screenshot({path:'artifacts/riding-social.png'});
